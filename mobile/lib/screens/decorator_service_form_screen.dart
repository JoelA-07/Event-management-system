...
class DecoratorServiceFormScreen extends StatefulWidget {
  final VendorModel? existing;
  const DecoratorServiceFormScreen({super.key, this.existing});

  @override
  State<DecoratorServiceFormScreen> createState() => _DecoratorServiceFormScreenState();
}

class _DecoratorServiceFormScreenState extends State<DecoratorServiceFormScreen> {
  ...
  Future<void> _save() async {
    ...
    final data = FormData.fromMap({
      ...
      if (_selectedImage != null)
        "image": await MultipartFile.fromFile(_selectedImage!.path, filename: "decor_${DateTime.now().millisecondsSinceEpoch}.jpg"),
    });
    ...
    if (res?.statusCode == 201 || res?.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(res?.data['message'] ?? "Failed to save service")),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.existing == null ? "New Decor Package" : "Edit Decor Package")),
          ...
          child: Column(
            children: [
              ...
              _isSaving
                  ...
                  : ElevatedButton(
                      onPressed: _save,
                      child: Text(widget.existing == null ? "CREATE PACKAGE" : "SAVE CHANGES"),
                    ),
            ],
          ),
        ),
