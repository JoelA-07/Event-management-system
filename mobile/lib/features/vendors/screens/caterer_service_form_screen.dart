...
class CatererServiceFormScreen extends StatefulWidget {
  final VendorModel? existing;
  const CatererServiceFormScreen({super.key, this.existing});

  @override
  State<CatererServiceFormScreen> createState() => _CatererServiceFormScreenState();
}

class _CatererServiceFormScreenState extends State<CatererServiceFormScreen> {
  ...
  Future<void> _save() async {
    ...
    final data = FormData.fromMap({
      ...
      if (_selectedImage != null)
        "image": await MultipartFile.fromFile(_selectedImage!.path, filename: "cater_${DateTime.now().millisecondsSinceEpoch}.jpg"),
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.existing == null ? "New Catering Service" : "Edit Catering Service")),
          ...
          child: Column(
            children: [
              ...
              _isSaving
                  ...
                  : ElevatedButton(
                      onPressed: _save,
                      child: Text(widget.existing == null ? "CREATE SERVICE" : "SAVE CHANGES"),
                    ),
            ],
          ),
        ),
