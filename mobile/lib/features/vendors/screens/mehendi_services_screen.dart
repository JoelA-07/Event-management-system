...
import 'package:mobile/features/vendors/screens/mehendi_service_form_screen.dart';

class MehendiServicesScreen extends StatefulWidget {
  const MehendiServicesScreen({super.key});

  @override
  State<MehendiServicesScreen> createState() => _MehendiServicesScreenState();
}

class _MehendiServicesScreenState extends State<MehendiServicesScreen> {
  ...
  Future<void> _loadServices() async {
    ...
    if (!mounted) return;
    setState(() {
      _services = services.where((s) => s.category == 'mehendi').toList();
      _isLoading = false;
    });
  }
  ...
  Future<void> _openForm({VendorModel? existing}) async {
    final saved = await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => MehendiServiceFormScreen(existing: existing)),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Mehendi Designs")),
